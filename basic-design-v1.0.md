...
| 通知方針 | `human_review_required` / `system_degraded` / breaker open 時のみ Discord DM |
...
6. branch 排他は lock ではなく lease である。lease 喪失後の作業継続は禁止。
...
| ToolRunner (親プロセス) | subprocess 起動、lease renew、log 収集、operational metadata 更新、internal event 発火 |
...
| ToolRunner | task ごとに 1 親プロセス。並列 task 数の上限は運用設定 | branch lease で排他 |
...
- `tasks`: task truth。state, state_rev, critical, manual_gate_required, resume_target_state 等。
...
- `branch_waiters` / `branch_tokens`: branch 排他 (wait queue と fencing token)。
...
- Litestream WAL ship 間隔は 10 秒以下。
- journal は別ディスクに日単位 rotate で 30 日保持。オフサイト二重化は 7 日。
...
- SQLite primary / WAL replica / ingress_journal / Redis AOF / Forgejo dump / secret file。
...
- 成功判定は replication lag / snapshot 鮮度 / journal sync lag による。
