...
| 項目 | 決定 |
|---|---|
| エージェント構成 | 3 分離 (Planning: Claude Code CLI (Claude Opus backend) / Execution: Claude Code CLI / Review: Codex GPT-5) |
...
- `supports_request_id=true` の adapter のみ、同一 `request_id` を再試行で継続利用する。
- Planner は Claude Code CLI subprocess を使うため `supports_request_id=false` とする。
